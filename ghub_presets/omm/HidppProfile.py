...
class Profile:
    ...
    def load_profile_bin(self, data):
        ...
        self.profile_name = str(file.read(48).decode("utf-16")).rstrip("\x00")
        #now at 160+48 = 208
    ...
    def profile_to_json(self):
        ...
        ret['chunk1'] = self.chunk1.decode('latin-1')

        ret['buttons'] = []
        for x in self.buttons:
            ret['buttons'].append(self._keymap_to_json(x))
        ret['buttons_padding'] = self.buttons_padding.decode('latin-1')

        ret['buttons_gshift'] = []
        for x in self.buttons_gshift:
            ret['buttons_gshift'].append(self._keymap_to_json(x)) 
        ret['buttons_gshift_padding'] = self.buttons_gshift_padding.decode('latin-1') 

        ret['rgb'] = []
        for rgb in self.rgb:
            ret['rgb'].append(self._rgb_to_json(rgb))
        #custom_animation_index + unused bytes
        ret['chunk2'] = self.chunk2.decode('latin-1')
    ...
    def profile_bytes_from_json(self, j, profile_index):
        ...
        ret += j['chunk1'].encode('latin-1')
        assert len(j['buttons']) * 4 + len(j['buttons_padding'].encode('latin-1')) == 64, f"wrong buttons size!"
